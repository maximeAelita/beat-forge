...
def main():
    ...
    autosave = os.path.join(ROOT, "projects", "_autosave.json")
    project = Project(autosave_path=autosave)

    url = "(web UI disabled)"
    httpd = None
    if not args.no_web:
        httpd, url = web_mod.start(project, ROOT, args.host, args.port,
                                   quiet=not args.verbose)

    runner = tools_mod.ToolRunner(project, ROOT, url)
